...
export function Profile({
	...
	return (
		<section
			key={user?.id || "profile-empty"}
			className='rounded-2xl border border-white/10 bg-[#151124] p-5 text-white'
		>
			<div className='mb-4 flex items-center justify-between'>
				<div>
					...
					<p className='mt-1 text-sm text-slate-300'>
						Update your name, gender, age, interests, and avatar.
					</p>
				</div>
			</div>
			...
			<div className='mb-4 rounded-xl border border-white/15 p-4 text-sm text-slate-200'>
				...
				<div className='grid grid-cols-1 gap-2 sm:grid-cols-2'>
					...
					<p>
						Gender: <span className='text-white'>{user?.gender || "-"}</span>
					</p>
					<p>
						Age:{" "}
					...
					<p>
						Interests:{" "}
						<span className='text-white'>{user?.interests?.length ?? 0}</span>
					</p>
					...
					<div className='flex-1'>
						<UploadButton
							...
							onClientUploadComplete={async (result) => {
								...
								if (!nextAvatarUrl) {
									setMessage(
										"Upload completed, but no avatar URL was returned.",
									);
									return;
								}
								...
								setMessage(
									ok
										...
										: "Avatar uploaded, but saving failed.",
								);
							}}
			...
			<form ref={formRef} onSubmit={handleSubmit} className='space-y-4'>
				<div className='grid grid-cols-1 gap-3 sm:grid-cols-2'>
					...
					<select
						name='gender'
						defaultValue={
							user?.gender || user?.demographics?.gender || "UNISEX"
						}
						className='rounded-lg border border-white/20 bg-white/5 px-3 py-2 text-sm outline-none'
					>
						<option value='UNISEX' className='bg-[#151124]'>
							UNISEX
						</option>
						<option value='MALE' className='bg-[#151124]'>
							MALE
						</option>
						<option value='FEMALE' className='bg-[#151124]'>
							FEMALE
						</option>
						<option value='OTHER' className='bg-[#151124]'>
							OTHER
						</option>
					</select>
					<input
				...
				<div className='rounded-xl border border-white/15 p-4 text-sm text-slate-200'>
					Publicly available try-ons:{" "}
					<span className='font-semibold text-white'>{publicTryonsCount}</span>
				</div>
