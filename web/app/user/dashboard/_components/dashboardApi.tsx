...
function normalizeProfile(payload: unknown): UserProfile | null {
	...
	const base = (
		sourceData.user && typeof sourceData.user === "object"
			? sourceData.user
			: sourceData
	) as Record<string, unknown>;
	const demographics =
	...
	return {
		...
		location:
			typeof base.location === "string"
				...
				: typeof demographics?.location === "string"
					...
					: null,
		emailVerified:
			typeof base.emailVerified === "boolean" ? base.emailVerified : undefined,
		...
		interests: Array.isArray(base.interests)
			...
			: [],
		userBodyImageUrl:
			typeof base.userBodyImageUrl === "string" ? base.userBodyImageUrl : null,
...
async function fetchJobMeta(
	jobId: string,
): Promise<{ imageUrl: string | null; productIds: string[] }> {
...
function buildMetrics(
	records: Array<TryonRecord & { resolvedProductIds: string[] }>,
): DashboardMetrics {
	...
	return {
		favouriteCount: records.filter((record) => Boolean(record.isFavorite))
			.length,
...
export async function fetchDashboardData(): Promise<DashboardData> {
	...
	const enriched = await Promise.all(
		tryonRecords.slice(0, 60).map(async (record, index) => {
			...
			return {
				...
				resolvedProductIds:
					nativeProductIds.length > 0 ? nativeProductIds : jobMeta.productIds,
				label: record.tryonType || `Try-on #${index + 1}`,
			};
		}),
	);
	...
	const publicTryonsCount = enriched.filter((record) =>
