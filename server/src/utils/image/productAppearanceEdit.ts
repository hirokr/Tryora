...
export type ProductAppearanceEditPollingUpdate = {
  requestId: string;
  status: string;
  pollCount: number;
...
export type ProductAppearanceEditAiHooks = {
  ...
  onPollingUpdate?: (
    update: ProductAppearanceEditPollingUpdate
  ) => Promise<void> | void;
};
...
const runHookSafely = async (
  ...
  try {
    ...
    logger.warn(`[Product Ai Edit] ${label} hook failed`, {
...
const buildAiEditStatusUrl = (payload: ClaidEditApiResponse) => {
  ...
  if (resultUrl) {
    try {
      ...
      // Claid can return an HTTP status URL; poll over HTTPS to keep auth attached.
      if (
...
const pollAiEditResultUrl = async (
  statusUrl: string,
  requestId: string,
  ...
  while (pollCount < CLAID_AI_EDIT_MAX_POLL_CHECKS) {
    ...
    const status = readAiEditStatus(payload) || 'PROCESSING';
    const outputUrl = readOutputUrl(payload);

    pollCount += 1;

    await runHookSafely(
      hooks?.onPollingUpdate
        ? () =>
            hooks.onPollingUpdate!({
              requestId,
              status,
              pollCount,
              maxPollChecks: CLAID_AI_EDIT_MAX_POLL_CHECKS,
            })
        : undefined,
      'Polling update'
    );
...
export const editProductImageWithAi = async (
  ...
  while (true) {
    ...
    const payload = (await response.json()) as ClaidEditApiResponse;
    const requestId = String(payload.data?.id ?? randomUUID());

    await runHookSafely(
      hooks?.onRequestAccepted
        ? () => hooks.onRequestAccepted!(requestId)
        : undefined,
      'Request accepted'
    );
    ...
    const outputUrl = await pollAiEditResultUrl(statusUrl, requestId, hooks);
    return { outputUrl };
  }
};
