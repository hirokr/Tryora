...
- Input payload: {user_id, front_photo_url, side_photo_url, back_photo_url}
...
- Input payload: {user_id, avatar_glb_url, dress_image_url, scene_prompt?}
...
- Input payload: {query, max_results?}
- Process: Serper API Google search → parse dress data
- Output: Update ai_jobs with status=COMPLETED

## Folder Structure (enforce strictly)

- `.claude/` local assistant settings and skills
- `app/` FastAPI app, Celery tasks, services, models, routers, and core utilities
- `prisma/` shared Prisma schema for PostgreSQL tables
- `tests/` pytest suite
- `.env.example` environment template
- `README.md` project setup and API/job docs
